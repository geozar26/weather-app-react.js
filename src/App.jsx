...
function App() {
  ...
  // Δυναμικό Background ανάλογα με τον καιρό
  const getBackground = () => {
  ...
  return (
    <div style={{ 
      ...
      padding: '40px 12px 100px 12px', // Μεγάλο περιθώριο κάτω για να μη κόβονται οι κάρτες
      ...
      <style>
        {`
          * { box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
          ...
          .error-text {
            color: #FFC107; font-weight: 700; font-size: 0.9rem; margin-top: 10px;
        ...
        <div className="intense-text" style={{ 
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '15px' }}>
              <div style={{ fontSize: '5.8rem', fontWeight: '900', lineHeight: 1 }}>
                {Math.round(weather.main.temp)}°
              </div>
              <h1 style={{ fontSize: '3.2rem', fontWeight: '900', margin: 0, lineHeight: 1 }}>
                {weather.name}
              </h1>
            </div>
...
function DetailTile({ label, icon, value, color }) {
  return (
    <div className="glass-tile" style={{ padding: '15px 5px', display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '-10%', left: '-10%', width: '30px', height: '30px', background: color, filter: 'blur(20px)', opacity: 0.2 }}></div>
      <span className="mi-icon-premium" style={{ color: color, textShadow: `0 0 10px ${color}88`, marginBottom: '5px' }}>{icon}</span>
