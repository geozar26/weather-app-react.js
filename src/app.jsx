...
function App() {
  ...
  const getLocalTime = (offset) => {
    ...
    return {
      date: formatText(nd.toLocaleDateString('el-GR', { weekday: 'long', day: 'numeric', month: 'long' })),
      time: nd.toLocaleTimeString('el-GR', { hour: '2-digit', minute: '2-digit' })
    };
  };
  ...
  const getWeather = async (cityName, isInitial = false) => {
    ...
    try {
      ...
      if (!isInitial) {
        setHistory(prev => [searchName, ...prev.filter(c => c.toLowerCase() !== searchName.toLowerCase())].slice(0, 10));
      }
      setCity("");
    } catch (err) { 
  ...
  const getBg = (main) => {
    switch(main) {
      ...
      case 'Rain': case 'Drizzle': return 'linear-gradient(to bottom, #4b6cb7, #182848)';
      case 'Thunderstorm': return 'linear-gradient(to bottom, #4834d4, #130f40)';
      case 'Snow': return 'linear-gradient(to bottom, #a1c4fd, #c2e9fb)';
      case 'Mist': case 'Smoke': case 'Haze': case 'Fog': return 'linear-gradient(to bottom, #bdc3c7, #2c3e50)';
      default: return 'linear-gradient(to bottom, #4c5c74, #2c3e50)';
    }
  };

  return (
    <div style={{ minHeight: '100vh', width: '100vw', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', background: weather ? getBg(weather.weather[0].main) : '#2c3e50', transition: 'background 0.8s ease', position: 'relative' }}>
      ...
      {weather && (
        <div style={{ textAlign: 'center', width: '100%', maxWidth: '450px', display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px' }}>
          <div>
            <div style={{ fontWeight: 900, fontSize: '0.9rem', marginBottom: 5, textShadow: '0 2px 4px rgba(0,0,0,0.2)' }}>
              {localTime.date} {localTime.time}
            </div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 15, alignItems: 'center', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '4.5rem', fontWeight: 900, textShadow: '0 2px 10px rgba(0,0,0,0.2)' }}>{Math.round(weather.main.temp)}°</span>
              <span style={{ fontSize: '2.2rem', fontWeight: 950, textShadow: '0 2px 5px rgba(0,0,0,0.2)' }}>{formatText(weather.name)}</span>
            </div>
            
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
               <MainWeatherIcon iconType={weather.weather[0].main} />
               <div style={{ fontSize: '1.2rem', fontWeight: 900, textShadow: '0 2px 4px rgba(0,0,0,0.2)' }}>{formatText(weather.weather[0].description)}</div>
            </div>
          </div>
          ...
          <div className="search-container" ref={dropdownRef}>
            <div className="search-wrapper">
              ...
              />
              {city && <span className="material-icons close-icon-btn" onClick={() => { setCity(""); setError(""); }}>close</span>}
              <button className="search-btn" onClick={() => getWeather(city)}>ΑΝΑΖΗΤΗΣΗ</button>
            </div>
            ...
            {(formatText(weather.name) !== formatText(DEFAULT_CITY)) && (
              <button className="back-btn" onClick={() => { setError(""); getWeather(DEFAULT_CITY, true); }}>
          ...
          <div className="forecast-container" style={{ display: 'flex', gap: 6 }}>
            {forecast.map((f, i) => (
              <div key={i} className="glass" style={{ flex: 1, padding: '12px 2px' }}>
                <div style={{ fontSize: '0.75rem', fontWeight: 950 }}>{formatText(new Date(f.dt_txt).toLocaleDateString('el-GR', {weekday: 'short'}))}</div>
                <img src={`https://openweathermap.org/img/wn/${f.weather[0].icon}.png`} alt="" width="38" />
                <div style={{ fontWeight: 950, fontSize: '1rem' }}>{Math.round(f.main.temp)}°</div>
              </div>
            ))}
          </div>

          <div className="detail-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
            <DetailTile label="ΑΙΣΘΗΤΗ" val={`${Math.round(weather.main.feels_like)}°`} icon="thermostat" col="#FFD700" />
            <DetailTile label="ΥΓΡΑΣΙΑ" val={`${weather.main.humidity}%`} icon="water_drop" col="#4dabf7" />
            <DetailTile label="ΑΝΕΜΟΣ" val={`${Math.round(weather.wind.speed)}m/s`} icon="air" col="#69db7c" />
            <DetailTile label="ΑΝΑΤΟΛΗ" val={new Date((weather.sys.sunrise + weather.timezone - 7200) * 1000).toLocaleTimeString('el-GR', {hour:'2-digit', minute:'2-digit'})} icon="wb_sunny" col="#ffd43b" />
            <DetailTile label="ΔΥΣΗ" val={new Date((weather.sys.sunset + weather.timezone - 7200) * 1000).toLocaleTimeString('el-GR', {hour:'2-digit', minute:'2-digit'})} icon="wb_twilight" col="#FF3D00" />
            <DetailTile label="ΑΤΜ ΠΙΕΣΗ" val={`${weather.main.pressure} hPa`} icon="speed" col="#00D4FF" />
          </div>
        </div>
      )}
    </div>
...
function MainWeatherIcon({ iconType }) {
  ...
  useEffect(() => { document.fonts.ready.then(() => setIsReady(true)); }, []);
  ...
  return (
    <i className={`material-icons tile-icon ${isReady ? 'ready' : ''}`} style={{ fontSize: '32px', color: iconType === 'Clear' ? '#ffd43b' : '#fff' }}>
...
function DetailTile({ label, val, icon, col }) {
  ...
  useEffect(() => { document.fonts.ready.then(() => setIsReady(true)); }, []);
  return (
    <div className="glass" style={{ padding: '15px 2px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <i className={`material-icons tile-icon ${isReady ? 'ready' : ''}`} style={{ color: col, fontSize: '24px', marginBottom: '5px' }}>{icon}</i>
      <div style={{ fontSize: '0.65rem', fontWeight: 950, color: 'white', marginBottom: '2px' }}>{label}</div>
